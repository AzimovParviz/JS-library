...
export default {
  ...
  getBorrowed: async (userId: string) => {
    try {
      const res = await axios.get(`${URL}/borrowed/${userId}`);
      return {
        data: res.data,
        status: res.status,
      };
  ...
  addBorrower: async (data: PutType) => {
    try {
      const { bookId, updatedBook } = data;
      const res = await axios.put(`${URL}/borrowed/${bookId}`, updatedBook, {
        headers: {
          Authorization: `Bearer ${localStorage.getItem("token")}`,
        },
      });
  ...
  removeBorrower: async (data: PutType) => {
    try {
      const { bookId, updatedBook } = data;
      const res = await axios.put(`${URL}/return/${bookId}`, updatedBook, {
        headers: {
          Authorization: `Bearer ${localStorage.getItem("token")}`,
        },
      });
